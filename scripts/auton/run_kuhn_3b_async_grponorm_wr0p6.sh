#!/bin/bash
#SBATCH --job-name=kuhn_3b_async_grponorm_wr0p6
#SBATCH --output=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_async_grponorm_wr0p6_%j.out
#SBATCH --error=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_async_grponorm_wr0p6_%j.err
...
python examples/start_agentic_pipeline.py \
    ...
    exp_name=kuhn_3b_async_grponorm_wr0p6 \
    tracker_kwargs.tags="[kuhn_poker,fsp_train,qwen2_5_3b,async,auton,grpo_norm,traj_group,wr0p6]" \
    2>&1

rm -rf $TMPDIR
echo "===== KUHN 3B ASYNC GRPONORM WR0P6 DONE (RUN_ID=${RUN_ID}) ====="
