#!/bin/bash
#SBATCH --job-name=kuhn_3b_stochastic_wr0p6
#SBATCH --output=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_stochastic_wr0p6_%j.out
#SBATCH --error=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_stochastic_wr0p6_%j.err
...
python examples/start_agentic_pipeline.py \
    ...
    train_env_manager.tags="[KuhnPokerLLMThinkStochastic]" \
    val_env_manager.tags="[KuhnPokerLLMThinkStochastic]" \
    fsp_win_rate_threshold=0.6 \
    fsp_win_rate_window=10 \
    actor_train.use_dynamic_batching_in_train=false \
    exp_name=kuhn_3b_stochastic_wr0p6 \
    tracker_kwargs.tags="[kuhn_poker,fsp_train,qwen2_5_3b,stochastic_policy,async,auton,wr0p6]" \
    2>&1

rm -rf $TMPDIR
echo "===== KUHN 3B STOCHASTIC WR0P6 DONE (RUN_ID=${RUN_ID}) ====="
