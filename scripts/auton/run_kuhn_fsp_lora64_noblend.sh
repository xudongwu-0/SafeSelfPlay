#!/bin/bash
#SBATCH --job-name=kuhn_fsp_lora64_noblend
#SBATCH --output=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_fsp_lora64_noblend_%j.out
#SBATCH --error=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_fsp_lora64_noblend_%j.err
...
python examples/start_agentic_pipeline.py \
    ...
    actor_infer.model_args.lora_rank=64 \
    actor_infer.model_args.lora_alpha=64 \
    exp_name=kuhn_fsp_lora64_noblend \
    tracker_kwargs.tags="[kuhn_poker,fsp_train,qwen2_5_3b,cold_start,async,auton,lora64,noblend,gs24]" \
    2>&1

rm -rf $TMPDIR
echo "===== KUHN FSP LORA64 NOBLEND DONE (RUN_ID=${RUN_ID}) ====="
