#!/bin/bash
#SBATCH --job-name=kuhn_3b_pr_stochastic
#SBATCH --output=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_pr_stochastic_%j.out
#SBATCH --error=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_pr_stochastic_%j.err
...
python examples/start_agentic_pipeline.py \
    ...
    exp_name=kuhn_3b_stochastic_async \
    tracker_kwargs.tags="[kuhn_poker,fsp_train,qwen2_5_3b,stochastic_policy,async,auton,preempt]" \
    2>&1

rm -rf $TMPDIR
echo "===== KUHN 3B PREEMPT STOCHASTIC ASYNC DONE (RUN_ID=${RUN_ID}) ====="
