#!/bin/bash
#SBATCH --job-name=kuhn_3b_pr_sync_dyn
#SBATCH --output=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_pr_sync_dyn_%j.out
#SBATCH --error=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_pr_sync_dyn_%j.err
...
python examples/start_agentic_pipeline.py \
    ...
    actor_train.use_dynamic_batching_in_train=true \
    actor_train.max_tokens_per_microbatch_in_train=8192 \
    exp_name=kuhn_3b_acttag_fix_sync_dynbatch \
    tracker_kwargs.tags="[kuhn_poker,fsp_train,qwen2_5_3b,cold_start,sync,auton,preempt,dynamic_batch]" \
    2>&1

rm -rf $TMPDIR
echo "===== KUHN 3B PREEMPT SYNC DYNBATCH DONE (RUN_ID=${RUN_ID}) ====="
