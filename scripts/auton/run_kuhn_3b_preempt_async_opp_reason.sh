#!/bin/bash
#SBATCH --job-name=kuhn_3b_pr_async_opp_reason
#SBATCH --output=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_pr_async_opp_reason_%j.out
#SBATCH --error=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_pr_async_opp_reason_%j.err
...
python examples/start_agentic_pipeline.py \
    ...
    train_env_manager.include_opponent_reasoning=true \
    actor_train.use_dynamic_batching_in_train=false \
    fsp_win_rate_threshold=0.6 \
    fsp_win_rate_window=10 \
    exp_name=kuhn_3b_async_opp_reason \
    tracker_kwargs.tags="[kuhn_poker,fsp_train,qwen2_5_3b,cold_start,async,auton,preempt,opp_reason,wr0p6]" \
    2>&1

rm -rf $TMPDIR
echo "===== KUHN 3B PREEMPT ASYNC OPP REASON DONE (RUN_ID=${RUN_ID}) ====="
