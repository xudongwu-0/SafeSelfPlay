#!/bin/bash
#SBATCH --job-name=kuhn_3b_gs32_ndbatch_grpnorm_wr0p55_pr
#SBATCH --output=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_gs32_ndbatch_grpnorm_wr0p55_pr_%j.out
#SBATCH --error=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_gs32_ndbatch_grpnorm_wr0p55_pr_%j.err
...
python examples/start_agentic_pipeline.py \
    ...
    train_env_manager.num_env_groups=9 \
    'train_env_manager.num_groups_partition=[9]' \
    rollout_batch_size=288 \
    actor_train.use_dynamic_batching_in_train=false \
    +reward_normalization.grouping=traj_group_id \
    +reward_normalization.norm_mean_type=group \
    +reward_normalization.norm_std_type=group \
    fsp_win_rate_threshold=0.55 \
    fsp_win_rate_window=10 \
    exp_name=kuhn_3b_gs32_ndbatch_grpnorm_wr0p55 \
    tracker_kwargs.tags="[kuhn_poker,fsp_train,qwen2_5_3b,cold_start,async,auton,preempt,gs32,ndbatch,grpnorm,wr0p55]" \
    2>&1

rm -rf $TMPDIR
echo "===== KUHN 3B GS32 NDBATCH GRPNORM WR0P55 PREEMPT DONE (RUN_ID=${RUN_ID}) ====="
