#!/bin/bash
#SBATCH --job-name=kuhn_3b_mnt_sync
#SBATCH --output=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_mnt_sync_%j.out
#SBATCH --error=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_mnt_sync_%j.err
...
MNT="${1:?Usage: sbatch $0 <max_num_batched_tokens>}"
...
FSP_OUTPUT_ROOT=/zfsauton/scratch/wentsec/kuhn_poker_output/runs/kuhn_3b_mnt${MNT}_sync_${RUN_ID}
...
python examples/start_agentic_pipeline.py \
    ...
    exp_name=kuhn_3b_sync_mnt${MNT} \
    tracker_kwargs.project=self-play-debug \
    tracker_kwargs.tags="[kuhn_poker,fsp_train,qwen2_5_3b,cold_start,sync,auton,mnt_sweep,mnt${MNT}]" \
    actor_infer.strategy_args.strategy_config.max_num_batched_tokens=${MNT} \
    2>&1

rm -rf $TMPDIR
echo "===== KUHN 3B SYNC MNT=${MNT} DONE (RUN_ID=${RUN_ID}) ====="
