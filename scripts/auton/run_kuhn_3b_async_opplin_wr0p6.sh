#!/bin/bash
#SBATCH --job-name=kuhn_3b_async_opplin_wr0p6
#SBATCH --output=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_async_opplin_wr0p6_%j.out
#SBATCH --error=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_async_opplin_wr0p6_%j.err
...
python examples/start_agentic_pipeline.py \
    ...
    exp_name=kuhn_3b_async_opplin_wr0p6_grpnorm \
    tracker_kwargs.tags="[kuhn_poker,fsp_train,qwen2_5_3b,cold_start,async,auton,opp_linear,wr0p6,grpnorm]" \
    2>&1

rm -rf $TMPDIR
echo "===== KUHN 3B ASYNC OPPLIN WR0P6 DONE (RUN_ID=${RUN_ID}) ====="
