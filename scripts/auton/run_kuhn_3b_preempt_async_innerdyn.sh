#!/bin/bash
#SBATCH --job-name=kuhn_3b_pr_async_dyn
#SBATCH --output=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_pr_async_dyn_%j.out
#SBATCH --error=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_pr_async_dyn_%j.err
...
python examples/start_agentic_pipeline.py \
    ...
    actor_train.use_inner_dynamic_batching_in_train=true \
    actor_train.sequence_length_round_in_train=64 \
    exp_name=kuhn_3b_lorafix_async_innerdyn \
    tracker_kwargs.tags="[kuhn_poker,fsp_train,qwen2_5_3b,cold_start,async,auton,preempt,inner_dynbatch]" \
    2>&1

rm -rf $TMPDIR
echo "===== KUHN 3B PREEMPT ASYNC INNERDYN DONE (RUN_ID=${RUN_ID}) ====="
