#!/bin/bash
#SBATCH --job-name=kuhn_3b_pr_async_kl0p03
#SBATCH --output=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_pr_async_kl0p03_%j.out
#SBATCH --error=/zfsauton/scratch/wentsec/ROLL/logs/kuhn_3b_pr_async_kl0p03_%j.err
...
python examples/start_agentic_pipeline.py \
    ...
    init_kl_coef=0.03 \
    kl_loss_coef=0.03 \
    exp_name=kuhn_3b_lorafix_async_kl0p03 \
    tracker_kwargs.tags="[kuhn_poker,fsp_train,qwen2_5_3b,cold_start,async,auton,preempt,kl0p03]" \
    2>&1

rm -rf $TMPDIR
echo "===== KUHN 3B PREEMPT ASYNC KL0P03 DONE (RUN_ID=${RUN_ID}) ====="
