...
A_SUFFIX="curriculum_p${POOL_SIZE}_one_round_${RUN_TAG}_A1"
D_SUFFIX="curriculum_p${POOL_SIZE}_one_round_${RUN_TAG}_D1_vs_A1"
A_RUN_NAME="upstream_selfredteam_attacker_lora_r32_fromSFT_vs_base_normalmix_p${POOL_SIZE}_s${STEPS}_rb${ROLLOUT_BATCH_SIZE}_mb${MICRO_TRAIN_BATCH_SIZE}_tb${TRAIN_BATCH_SIZE}_lr${ATTACKER_LR_TAG}_cosmin_nosft_${A_SUFFIX}"
A_ADAPTER="/output/upstream_selfredteam_role_lora/${A_RUN_NAME}/ckpt/global_step${STEPS}_hf"
DRIVER_LOG="/home/xudong/work/self_play/checkpoints/upstream_curriculum_one_round_${RUN_TAG}.driver.log"

{
  ...
  modal run \
    ...
    --steps "${STEPS}" \
    --normal-prompt-pool-size "${POOL_SIZE}" \
